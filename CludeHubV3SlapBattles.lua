...
local tool = player:WaitForChild("Backpack"):WaitForChild("Tool")  -- Default glove

-- All Hit Events from Your List
local slapEvents = {
    "GoldenHit", "MailHit", "Speedrunhit", "BoomerangH", "HtSpace", "MisterHit", 
    "GeneralHit", "ReaperHit", "ReplicaHit", "DefenseHit", "KSHit", "ReverseHit", 
    "ShukuchiHit", "DuelistHit", "woahHit", "IceHit", "hitAdios", "BlockedHit", 
    "engiehit", "RockyHit"
}

-- Glove Name Check
local function getGloveName()
    return tool.Name  -- Getting the name of the glove currently equipped
end

-- Configurable Range for the Slap Aura (in studs)
local slapAuraRange = 20  -- Change this to adjust the range
...
local function slapAndStunClosestPlayer()
    local events = {}
    -- Retrieve all events
    for _, eventName in ipairs(slapEvents) do
    ...
    if #events == 0 then return end  -- Exit if no events are found

    while slapAuraEnabled do
        -- Only fire slap events if the player has the glove equipped
        local gloveName = getGloveName()

        -- Only execute slap events if the glove matches the condition
        if gloveName == "YourGloveNameHere" then  -- Replace "YourGloveNameHere" with your glove name, or add a condition for default gloves
            for _, otherPlayer in pairs(Players:GetPlayers()) do
                if otherPlayer ~= player and otherPlayer.Character then
                    local otherHRP = otherPlayer.Character:FindFirstChild("HumanoidRootPart")
                    if otherHRP and (hrp.Position - otherHRP.Position).Magnitude <= slapAuraRange then
                        local args = {[1] = otherHRP}
                        -- Fire all the events simultaneously for the equipped glove
